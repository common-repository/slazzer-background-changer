<script>
    ...
    jQuery("input[name='slazzer_products']").change(function () {
        if (jQuery("input[name='slazzer_products']:checked").val() == 'specified')
            jQuery("#Slazzer_products_IDs").show();
        else
            jQuery("#Slazzer_products_IDs").hide();
    });
    jQuery("#slazzer_save_settings").on('click', function () {
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (data == -1) {
                    jQuery("#remaining_credits").text("Invalid API key");
                    jQuery("#remaining_credits").css('color', 'red');
                } else {
                    jQuery("#remaining_credits").text("Remaining Credits: " + data);
    ...
    jQuery("#slazzer_start_background_removal").on('click', function () {
        ...
        var ajaxurl = "<?php if ( ! defined( 'ABSPATH' ) ) exit; echo esc_url(admin_url('admin-ajax.php')); ?>";
        jQuery.ajax({
            url: ajaxurl,
            type: "POST",
            data: formdata,
            ...
            success: function (data) {
                if (data.status == false) {
                    ...
                    jQuery(':input[type="button"]').css('cursor', 'pointer');
                    alert(data.message);
                    // $('#_slazzer_ui_notificatuion').css('display', '');
                    // $('#_slazzer_ui_notificatuion').text("<b>Note: </b>"+data.message);
                    return false;
                }
    ...
    function call_ajax_sync() {
           ...
           // var product_id = data[index].product_id;
           // var image_id = data[index].image_id;
           // var type = data[index].type;
            ...
            // form_data_processsingleimage.append('product_id', product_id);
            // form_data_processsingleimage.append('image_id', image_id);
            // form_data_processsingleimage.append('type', type);
            ...
            thisajaxquery = jQuery.ajax({
                ...
                success: function (res) {
                    ...
                    // var remaining_credits =  data.remaining_credits;
                    if (status == false) {
                        
                        jQuery("#slazzer-loader").hide();
                        var thismessage = res.message;
                        // console.log('status' + status);
                        // tempAlert(thismessage, 5000);
                        jQuery(':input[type="button"]').prop('disabled', false);
                        jQuery(':input[type="button"]').css('cursor', 'pointer');
                        location.reload();
                        // return false;
    ...
    //     // jQuery('#_slazzer_ui_notificatuion').css('display', '');
    //     // jQuery('#slazzer_ui_notificatuion').html(message);
    //     jQuery('#slazzer_ui_notificatuion').text("Note: "+message);
    // }
